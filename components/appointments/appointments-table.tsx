...
import { Check, X, Eye, Search, Filter, CalendarX, Phone, Clock, FileText, User, Calendar } from 'lucide-react';
        ...
        <DialogContent className="max-w-2xl w-full rounded-2xl p-0 overflow-hidden gap-0 bg-background max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-3 sm:gap-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    <Badge variant="outline" className={cn('rounded-lg text-xs', status.className)}>
                      {status.label}
                    </Badge>
                    <span className="text-xs text-muted-foreground">
                    Booked {formatDistanceToNow(new Date(appointment.createdAt), { addSuffix: true })}
                  </span>
          ...
          <div className="px-4 sm:px-8 py-5 sm:py-6 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 bg-background">
            ...
            <div className="space-y-3">
              ...
              <DetailItem icon={<Phone className="w-4 h-4" />} label="Phone" value={appointment.patientPhone} />
              <DetailItem icon={<Calendar className="w-4 h-4" />} label="Date" value={format(new Date(appointment.date), 'EEEE, MMM d yyyy')} />
              <DetailItem icon={<Clock className="w-4 h-4" />} label="Time" value={appointment.time} />
              <DetailItem icon={<User className="w-4 h-4" />} label="Doctor" value={`Dr. ${appointment.doctorName ?? 'Assigned'}`} />
            </div>
            ...
            {appointment.status === 'pending' && (
                <>
                  <Button
                      ...
                      onClick={() => { onReject(appointment); onOpenChange(false); }}
                  >
                  ...
                  <Button
                      ...
                      onClick={() => { onAccept(appointment); onOpenChange(false); }}
                  >
...
export function AppointmentsTable() {
  ...
  const { getAppointmentsForDoctor, updateAppointmentStatus } = useAppointments();

  const allAppointments = currentDoctor ? getAppointmentsForDoctor(currentDoctor.id) : [];
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tr className="border-b border-border bg-muted">
                {['Patient', 'Phone', 'Issue', 'Date', 'Status', 'Actions'].map((h, i) => (
                    <th
                        key={h}
                        className={cn(
                            ...
                            <td className="px-4 sm:px-6 py-4">
                              <div className="flex items-center justify-end gap-1.5">
                                <Button
                                    ...
                                    onClick={(e) => { e.stopPropagation(); openDialog(appointment); }}
                                >
